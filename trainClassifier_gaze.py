# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 14:31:18 2017
...
#number of classes is 2 (gaze and nogaze)
nClass=2
...
vlabel, vimage = getImage("validation-00000-of-00001")
...
if simpleModel:
  ...
  W_fc1 = weight_variable([int((width/4)) * int((height/4)) * nFeatures2, nNeuronsfc])
  b_fc1 = bias_variable([nNeuronsfc])
  
  # flatten output from previous layer
  h_pool2_flat = tf.reshape(h_pool2, [-1, int((width/4)) * int((height/4)) * nFeatures2])
...
for i in range(nSteps):
    ...
    if (i+1)%10 == 0: # then perform validation 
      ...
      lgls=logloss(1,train_accuracy)
      print("step %d, training accuracy %g"%(i+1, train_accuracy))
      print('log loss   '+str(lgls))
